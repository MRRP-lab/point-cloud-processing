...
with dai.Device(pipeline) as device:
    # device.setIrLaserDotProjectorIntensity(0) # Value between [0, 1]
    ...
    while isRunning:
        ...
        if key == ord('q'):
                ...
                frame_counter += 1
                # Save RGB frame
                rgb_filename = save_dir / f"color_{frame_counter:04}.png"
                cv2.imwrite(str(rgb_filename), cvColorFrame)
                print(f"color frame saved: {rgb_filename}")

                # Save point cloud
                if len(pcd.points) > 0:
                        ply_filename = save_dir / f"pointcloud_{frame_counter:04}.ply"
                        o3d.io.write_point_cloud(str(ply_filename), pcd)
